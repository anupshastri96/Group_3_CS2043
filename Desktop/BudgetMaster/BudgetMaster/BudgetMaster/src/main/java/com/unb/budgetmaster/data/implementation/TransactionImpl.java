...
public class TransactionImpl implements TransactionABS{
    ...
    @Override
    public void addTransaction(LocalDate date, int id, double amount, String type, String category) {
        try{
            Statement statement = connection.createStatement();
    ...
    @Override
    public ArrayList<Transaction> getTransactions(String transactionType) {
        ...
        try{
            String transactionQuery = "select transaction_id, transaction_name, transaction_amount, date_format(transaction_date, '%d-%m-%Y'), " +
                    "transaction_payee from transaction_data natural join category_data natural join transaction_type where user_name = " +
                    Database.user.getUsername() +  "' and transaction_type = '" + transactionType + "' order by transaction_date asc;";
    ...
    @Override
    public ArrayList<Transaction> getTransactionsByCategory(String type, Category category) {
        ArrayList<Transaction> allTransactions = new ArrayList<Transaction>();
        Transaction transaction;
        int transactionID = 0;
        double transactionAmount = 0;
        ...
        try{
            String transactionQuery = "select transaction_id, transaction_type, transaction_name, transaction_amount, date_format(transaction_date" +
                    ", '%d-%m-%Y'), " +
                    "transaction_payee from transaction_data natural join category_data natural join transaction_type where user_name = " +
                    Database.user.getUsername() +  "' and category_name = '" + categoryName + "' order by transaction_date asc;";
            PreparedStatement preparedStatement = connection.prepareStatement(transactionQuery);
            ResultSet results = preparedStatement.executeQuery();
