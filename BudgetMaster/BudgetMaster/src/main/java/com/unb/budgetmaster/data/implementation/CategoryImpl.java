...
public class CategoryImpl implements CategoryABS{
    ...
    @Override
    public Boolean checkCategoryExists(String category, String transactionType) {
        Boolean success = false;
        query = "select " + category + " from category_data where transaction_name = '" + transactionType + "' and user_name = '" + Database.user.getUsername() + "';";
        try {
            statement = connection.prepareStatement(query);
            ResultSet results = statement.executeQuery();
            if(results.next()){
